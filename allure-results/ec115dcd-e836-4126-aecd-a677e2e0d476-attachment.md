...
Call log:
  - waiting for locator('select[ng-model="Visit.DepartmentID"]')
    - locator resolved to <select ng-model="Visit.DepartmentID" ng-disabled="isEmergencyDept" ng-options="item.value as item.text for item in drpDepartment" class="form-control input-sm ng-pristine ng-untouched ng-valid" ng-change="fnSetDoctor();fnDeparmentChange(1);fnSetCabin();onPricingpolicyChange();">…</select>
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 18/05/2026 12:14:12"
            - listitem [ref=e13]:
...
```ts
  ...
  254 |         // Using a text locator to find the tab/accordion header
  255 |         // .first() handles cases where the text might appear in multiple places (like a breadcrumb)
  256 |         const visitInfoTab = this.page.locator('text="Visit Information"').first();
  257 |         await visitInfoTab.waitFor({ state: 'visible' });
  258 |         await visitInfoTab.click();
  259 |         
  260 |         // Give Angular time to run its animation and render the fields inside the tab
  261 |         await this.page.waitForTimeout(500); 
  262 |     }
  263 | 
  264 |     // NEW METHOD: Fill Visit Information
  265 |     async fillVisitInformation(encounterType: string, department: string, doctor: string, admissionSource: string) {
  266 |         
  267 |         // 1. Encounter Type
  268 |         const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  269 |         await encounterDropdown.waitFor({ state: 'visible' });
  270 |         await encounterDropdown.selectOption({ label: encounterType });
  271 |         await this.page.waitForTimeout(500);
  272 | 
  273 |         // 2. Department (Triggers fnSetDoctor and fnSetCabin)
  274 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]');
  275 |         await departmentDropdown.waitFor({ state: 'visible' });
> 276 |         await departmentDropdown.selectOption({ label: department });
      |                                  ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  277 |         await departmentDropdown.dispatchEvent('change');
  278 |         await this.page.waitForTimeout(1000); // Give Angular time to load the specific doctors for this department
  279 | 
  280 |         // 3. Primary Doctor
  281 |         const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]');
  282 |         await doctorDropdown.waitFor({ state: 'visible' });
  283 |         
  284 |         // If your doctor string has trailing spaces in the HTML (e.g., "Doctor 1  "), 
  285 |         // using { label: doctor } will strictly match it.
  286 |         await doctorDropdown.selectOption({ label: doctor });
  287 |         await doctorDropdown.dispatchEvent('change');
  288 |         await this.page.waitForTimeout(1000);
  289 | 
  290 |         // 4. Admission Source
  291 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  292 |         await sourceDropdown.waitFor({ state: 'visible' });
  293 |         await sourceDropdown.selectOption({ label: admissionSource });
  294 |         await sourceDropdown.dispatchEvent('change');
  295 |         await this.page.waitForTimeout(500);
  296 | 
  297 |         // 5. Queue No. 
  298 |         // (Note: The HTML shows ng-disabled="iseditQueueNo==false", meaning this might be auto-generated.
  299 |         // We will check if it's enabled before trying to type into it.)
  300 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  301 |         if (await queueInput.isEnabled()) {
  302 |             const randomQueue = Math.floor(1000 + Math.random() * 9000).toString();
  303 |             await queueInput.fill(randomQueue);
  304 |         }
  305 |     }
  306 | }
```