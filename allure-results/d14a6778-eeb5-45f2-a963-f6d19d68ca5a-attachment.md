      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 02:35:05"
            - listitem [ref=e13]:
            ...
            - generic [ref=e102]:
              ...
              - generic [ref=e129]:
                ...
                - combobox [ref=e132]:
                  ...
                  - option "Anatomical Pathology" [selected]
              ...
              - generic [ref=e144]:
                ...
                - combobox [ref=e147]:
                  ...
                  - option "Medical Officer_Tieba"
                  - option "Nor Liza Ariffin" [selected]
...
```ts
  ...
  291 |     targetVisitType: string = 'New'
  292 | ) {
  293 |     const startTime = Date.now();
  294 | 
  295 |     // Partial names to search for - order doesn't matter, will shuffle
  296 |     const doctorKeywords = [
  297 |         'Doctor 12',
  298 |         'Doctor 52', 
  299 |         'Doctor_Annie',
  300 |         'Doctor 81',
  301 |         'Nor Liza Ariffin'
  302 |     ];
  303 | 
  304 |     // 1. ENCOUNTER TYPE
  305 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  306 |     await encounterDropdown.waitFor({ state: 'visible' });
  307 |     await encounterDropdown.selectOption({ label: encounterType });
  308 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  309 |     console.log('[Visit] 1. Encounter Type set');
  310 | 
  311 |     // 2. PRIMARY DOCTOR - Get all options and find matches
  312 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  313 |     await expect.poll(async () => doctorDropdown.locator('option').count()).toBeGreaterThan(1);
  314 |     
  315 |     // Get all option texts and values
  316 |     const allOptions = await doctorDropdown.locator('option').all();
  317 |     const matchedDoctors: { value: string, label: string }[] = [];
  318 |     
  319 |     for (let i = 1; i < allOptions.length; i++) {
  320 |         const value = await allOptions[i].getAttribute('value');
  321 |         const label = await allOptions[i].innerText();
  322 |         if (!value ||!label) continue;
  323 |         
  324 |         // Check if this option matches any of our keywords
  325 |         const matched = doctorKeywords.find(kw => label.includes(kw));
  326 |         if (matched) {
  327 |             matchedDoctors.push({ value, label: label.trim() });
  328 |             console.log(`[Visit] Matched: "${label.trim()}"`);
  329 |         }
  330 |     }
  331 | 
  332 |     if (matchedDoctors.length === 0) {
  333 |         throw new Error('CRITICAL: None of the target doctors found in dropdown');
  334 |     }
  335 | 
  336 |     // Shuffle the matched doctors
  337 |     for (let i = matchedDoctors.length - 1; i > 0; i--) {
  338 |         const j = Math.floor(Math.random() * (i + 1));
  339 |         [matchedDoctors[i], matchedDoctors[j]] = [matchedDoctors[j], matchedDoctors[i]];
  340 |     }
  341 |     console.log(`[Visit] Randomized order: ${matchedDoctors.map(d => d.label).join(' → ')}`);
  342 | 
  343 |     // Select first doctor to trigger department load
  344 |     const primaryDoctor = matchedDoctors[0];
  345 |     await doctorDropdown.selectOption(primaryDoctor.value); // Use value, not label
  346 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  347 |     console.log(`[Visit] 2. Primary Doctor set: ${primaryDoctor.label}`);
  348 | 
  349 |     // 3. Confirm Department loaded
  350 |     const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  351 |     await expect.poll(async () => departmentDropdown.inputValue(), { timeout: 5000 }).not.toBe('0');
  352 |     const deptText = await departmentDropdown.locator('option:checked').innerText();
  353 |     console.log(`[Visit] 3. Department auto-loaded: ${deptText}`);
  354 | 
  355 |     // 4. Loop matched doctors until Visit Type "New" found
  356 |     let foundDoctor = '';
  357 |     
  358 |     for (let i = 0; i < matchedDoctors.length; i++) {
  359 |         const doc = matchedDoctors[i];
  360 |         console.log(`[Visit] 4.${i+1} Checking ${doc.label} for "${targetVisitType}"...`);
  361 | 
  362 |         const currentDoctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
> 363 |         await currentDoctorDropdown.selectOption(doc.value); // Use value
      |                                     ^ Error: locator.selectOption: Target page, context or browser has been closed
  364 |         
  365 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  366 |         
  367 |         const hasOptions = await this.page.waitForFunction(
  368 |             (sel) => {
  369 |                 const el = document.querySelector(sel) as HTMLSelectElement;
  370 |                 return el && el.options.length > 1;
  371 |             },
  372 |             'select[ng-model="Visit.VisitType"]',
  373 |             { timeout: 3000 }
  374 |         ).then(() => true).catch(() => false);
  375 | 
  376 |         if (!hasOptions) {
  377 |             console.log(`❌ [Visit] No Visit Types for ${doc.label}`);
  378 |             continue;
  379 |         }
  380 | 
  381 |         const types = await visitTypeDropdown.locator('option').allInnerTexts();
  382 |         const cleanTypes = types.map(t => t.trim()).filter(t => t && t!== '--Select--');
  383 | 
  384 |         if (cleanTypes.includes(targetVisitType)) {
  385 |             console.log(`✅ [Visit] FOUND! ${doc.label} supports "${targetVisitType}"`);
  386 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  387 |             foundDoctor = doc.label;
  388 |             break;
  389 |         }
  390 | 
  391 |         console.log(`❌ [Visit] ${doc.label} only has: [${cleanTypes.join(', ')}]`);
  392 |     }
  393 | 
  394 |     if (!foundDoctor) {
  395 |         throw new Error(`CRITICAL: None of the matched doctors support "${targetVisitType}"`);
  396 |     }
  397 | 
  398 |     // 5. Queue No.
  399 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  400 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  401 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  402 |     await queueInput.fill(randomQueue);
  403 |     console.log(`[Visit] 5. Queue No: ${randomQueue}`);
  404 | 
  405 |     // 6. Admission Source
  406 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  407 |     await sourceDropdown.selectOption({ label: admissionSource });
  408 |     console.log(`[Visit] 6. Admission Source: ${admissionSource}`);
  409 | 
  410 |     console.log(`[Visit] Complete in ${Date.now() - startTime}ms ✅ Final Doctor: ${foundDoctor}`);
  411 | }
  412 | 
  413 | }
```