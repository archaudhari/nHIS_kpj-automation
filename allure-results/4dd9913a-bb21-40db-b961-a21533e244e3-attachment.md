      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 12:25:18"
            - listitem [ref=e13]:
...
```ts
  ...
  292 |     ) {
  293 |         const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  294 |         await encounterDropdown.waitFor({ state: 'visible' });
  ...
  298 |         
  299 |         // Wait for doctor dropdown to populate after encounter type
  300 |         await expect.poll(
  301 |             async () => doctorDropdown.locator('option').count(),
  302 |             { timeout: 8000, message: 'Doctor dropdown never populated' }
  303 |         ).toBeGreaterThan(1);
  304 |  
  305 |         const doctorOptionsCount = await doctorDropdown.locator('option').count();
  306 |         const availableDoctors: { name: string; value: string }[] = [];
  307 |         for (let i = 1; i < doctorOptionsCount; i++) {
  308 |             const opt = doctorDropdown.locator('option').nth(i);
  309 |             const name = (await opt.innerText()).trim();
  310 |             const value = await opt.getAttribute('value');
  311 |             if (value) availableDoctors.push({ name, value });
  312 |         }
  313 | 
  314 |         // ── 3. DOCTOR LOOP: pick first doctor that exposes Visit Type "New" ─
  315 | let foundWorkingDoctor = false;
  316 | 
  317 | for (const doctor of availableDoctors) {
  318 |     console.log(`[Visit] Trying doctor: "${doctor.name}"...`);
  319 | 
  320 |     // 1. Re-locate dropdown every iteration
  321 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
> 322 |     await doctorDropdown.waitFor({ state: 'visible', timeout: 5000 });
      |                          ^ Error: locator.waitFor: Target page, context or browser has been closed
  323 | 
  324 |     // 2. Select + wait for Angular to finish its navigation/re-render
  325 |     await Promise.all([
  326 |         // This waits for network to be idle after the change event fires
  327 |         this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {}),
  328 |         // Some Angular apps use hash routing, so also wait for URL to settle
  329 |         this.page.waitForURL('**/VisitScreen*', { timeout: 10000 }).catch(() => {}),
  330 |         doctorDropdown.selectOption(doctor.value)
  331 |     ]);
  332 | 
  333 |     // 3. Page may have re-rendered. Wait a beat for Angular to bootstrap
  334 |     await this.page.waitForTimeout(1000);
  335 | 
  336 |     // 4. Re-query EVERYTHING after potential reload
  337 |     const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  338 |     
  339 |     try {
  340 |         // Wait for department to auto-load
  341 |         await expect.poll(
  342 |             async () => {
  343 |                 const selected = await departmentDropdown.inputValue();
  344 |                 return selected !== '' && selected !== '0';
  345 |             },
  346 |             { timeout: 10000, message: `Department did not auto-load for doctor "${doctor.name}"` }
  347 |         ).toBe(true);
  348 | 
  349 |         const autoLoadedDept = await departmentDropdown.locator('option:checked').innerText();
  350 |         console.log(`[Visit] Department auto-loaded → "${autoLoadedDept.trim()}"`);
  351 |     } catch (e) {
  352 |         console.log(`[Visit] ⚠ Department did not auto-load for "${doctor.name}". ${e}`);
  353 |         continue; // try next doctor
  354 |     }
  355 | 
  356 |     // 5. Check Visit Type - re-query locator again
  357 |     const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  358 |     
  359 |     try {
  360 |         await expect.poll(
  361 |             async () => visitTypeDropdown.locator('option').count(),
  362 |             { timeout: 8000 }
  363 |         ).toBeGreaterThan(1);
  364 | 
  365 |         const rawTypes = await visitTypeDropdown.locator('option').allInnerTexts();
  366 |         const cleanTypes = rawTypes.map(v => v.trim());
  367 | 
  368 |         if (cleanTypes.includes(targetVisitType)) {
  369 |             console.log(`✅ [Visit] "${doctor.name}" supports Visit Type: "${targetVisitType}" — selecting & stopping.`);
  370 |             
  371 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  372 |             await this.page.waitForTimeout(300);
  373 | 
  374 |             foundWorkingDoctor = true;
  375 |             break; // >>>> Then Stop There
  376 |         } else {
  377 |             console.log(`❌ [Visit] "${targetVisitType}" not in [${cleanTypes.join(', ')}] for "${doctor.name}".`);
  378 |         }
  379 | 
  380 |     } catch {
  381 |         console.log(`❌ [Visit] Visit Type dropdown stayed empty for "${doctor.name}".`);
  382 |     }
  383 | }
  384 | 
  385 | if (!foundWorkingDoctor) {
  386 |     throw new Error(`CRITICAL: No doctor found that supports Visit Type "${targetVisitType}".`);
  387 | }
  388 |  
  389 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  390 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  391 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  392 |         await queueInput.fill(randomQueue);
  393 |         await expect(queueInput).toHaveValue(randomQueue);
  394 |  
  395 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  396 |         await sourceDropdown.waitFor({ state: 'visible' });
  397 |         await sourceDropdown.selectOption({ label: admissionSource });
  398 |         await expect(sourceDropdown).toHaveValue(/.+/);
  399 |     }
  400 | 
  401 | }
```