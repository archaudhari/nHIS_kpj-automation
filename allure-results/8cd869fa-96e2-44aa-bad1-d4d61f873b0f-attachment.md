...
Call log:
  - waiting for locator('select[ng-model="Visit.PrimaryDoctorID"]') to be visible
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 01:29:51"
            - listitem [ref=e13]:
...
```ts
  ...
  340 | 
  341 |     // 2. PRIMARY DOCTOR - Select different doctor each run
  342 | const primaryDoctorDropdown = this.page.locator('select[ng-model="Visit.PrimaryDoctorID"]');
> 343 | await primaryDoctorDropdown.waitFor({ state: 'visible', timeout: 5000 });
      |                             ^ TimeoutError: locator.waitFor: Timeout 5000ms exceeded.
  344 | 
  345 | // Wait for dropdown to have real options, not just "--Select--"
  346 | await this.page.waitForFunction((sel) => {
  347 |     const dropdown = document.querySelector(sel);
  348 |     return dropdown && dropdown.querySelectorAll('option').length > 1;
  349 | }, 'select[ng-model="Visit.PrimaryDoctorID"]', { timeout: 10000 });
  350 | 
  351 | // Get all options with value + text so we can filter properly
  352 | const doctorOptions = await primaryDoctorDropdown.locator('option').evaluateAll(opts =>
  353 |     opts
  354 |       .map(o => ({
  355 |             value: (o as HTMLOptionElement).value,
  356 |             text: o.textContent?.trim() || ''
  357 |         }))
  358 |       .filter(opt => opt.value && opt.value!== 'string:' && opt.text &&
  359 |               !opt.text.toLowerCase().includes('select')) // catches --Select--, Select Doctor, etc
  360 | );
  361 | 
  362 | console.log('[Visit] Primary Doctor options:', doctorOptions.map(d => d.text));
  363 | 
  364 | if (doctorOptions.length === 0) {
  365 |     throw new Error('No Primary Doctor options found in dropdown');
  366 | }
  367 | 
  368 | // Random selection each run
  369 | const randomIndex = Math.floor(Math.random() * doctorOptions.length);
  370 | const selectedDoctor = doctorOptions[randomIndex];
  371 | 
  372 | await primaryDoctorDropdown.selectOption(selectedDoctor.value);
  373 | 
  374 | // Angular often triggers change events - wait for network to settle
  375 | await this.page.waitForLoadState('networkidle', { timeout: 5000 });
  376 | console.log(`[Visit] 2. Primary Doctor set: ${selectedDoctor.text}`);
  377 |     // 5. Queue No.
  378 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  379 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  380 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  381 |     await queueInput.fill(randomQueue);
  382 |     console.log(`[Visit] 5. Queue No: ${randomQueue}`);
  383 | 
  384 |     // 6. Admission Source
  385 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  386 |     await sourceDropdown.selectOption({ label: admissionSource });
  387 |     console.log(`[Visit] 6. Admission Source: ${admissionSource}`);
  388 | }
  389 | }
```