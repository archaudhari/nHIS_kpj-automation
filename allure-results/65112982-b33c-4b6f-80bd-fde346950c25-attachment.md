...
Call log:
  - waiting for locator('select[ng-model="AppPatientData.AppDoctorModalityID"]') to be visible
    44 × locator resolved to hidden <select id="DoctorModality" name="DoctorModality" ng-disabled="IsDepartment" ng-model="AppPatientData.AppDoctorModalityID" class="form-control input-sm ng-pristine ng-untouched ng-valid" ng-options="item.value as item.text for item in ddlDoctorModality">…</select>
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 02:13:28"
            - listitem [ref=e13]:
              ...
              - generic [ref=e102]:
                ...
                - generic [ref=e129]:
                  ...
                  - combobox [ref=e132]:
                    ...
                    - option "Haemodalysis"
                    - option "Hand & Microsurgery (Orthopaedic)" [selected]
...
> 365 |     await doctorDropdown.waitFor({
  ...
  440 | 
  441 |     // ─────────────────────────────────────────────────────────────
  442 |     // 4. VISIT TYPE DROPDOWN
  443 |     // ─────────────────────────────────────────────────────────────
  444 |     const visitTypeDropdown = this.page.locator(
  445 |         'select[ng-model="AppPatientData.VisitTypeID"]'
  446 |     );
  447 | 
  448 |     let foundWorkingDoctor = false;
  449 | 
  450 |     // Shuffle doctors randomly
  451 |     const shuffledDoctors = [...availableDoctors]
  452 |         .sort(() => Math.random() - 0.5);
  453 | 
  454 |     // ─────────────────────────────────────────────────────────────
  455 |     // 5. LOOP THROUGH DOCTORS
  456 |     // ─────────────────────────────────────────────────────────────
  457 |     for (const doctor of shuffledDoctors) {
  458 | 
  459 |         console.log(
  460 |             `[Visit] Trying doctor → ${doctor.text}`
  461 |         );
  462 | 
  463 |         // Select doctor
  464 |         await doctorDropdown.selectOption({
  465 |             value: doctor.value
```