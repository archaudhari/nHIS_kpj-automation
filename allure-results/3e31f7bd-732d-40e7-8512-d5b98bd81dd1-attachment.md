      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 12:54:00"
            - listitem [ref=e13]:
              ...
              - generic [ref=e102]:
                ...
                - generic [ref=e129]:
                  ...
                  - combobox [ref=e132]:
                    ...
                    - option "Dermatology"
                    - option "Dietetics"
                    - option "Ear, Nose & Throat Surgery" [selected]
...
```ts
  ...
  292 | ) {
  293 |     // 1. ENCOUNTER TYPE
  294 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  295 |     await encounterDropdown.selectOption({ label: encounterType });
  296 |     await this.page.waitForLoadState('networkidle');
  297 | 
  298 |     // 2. Get all doctors once
  299 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  300 |     await expect.poll(async () => doctorDropdown.locator('option').count()).toBeGreaterThan(1);
  301 | 
  302 |     const options = await doctorDropdown.locator('option').all();
  303 |     const availableDoctors: { value: string, name: string }[] = [];
  304 |     for (let i = 1; i < options.length; i++) {
  305 |         const value = await options[i].getAttribute('value');
  306 |         const name = await options[i].innerText();
  307 |         if (value) availableDoctors.push({ value, name: name.trim() });
  308 |     }
  309 | 
  310 |     let foundWorkingDoctor = false;
  311 | 
  312 |     // 3. Try each doctor
  313 |     for (const doctor of availableDoctors) {
  314 |         console.log(`[Visit] Trying doctor: "${doctor.name}"...`);
  315 | 
  316 |         // Get fresh page reference in case previous iteration navigated
  317 |         const pages = this.page.context().pages();
  318 |         const currentPage = pages[pages.length - 1]; // last page = active one
  319 | 
> 320 |         const currentDoctorDropdown = currentPage.locator('select[ng-model="Visit.DoctorID"]').first();
      |                                                   ^ TypeError: Cannot read properties of undefined (reading 'locator')
  321 |         
  322 |         try {
  323 |             await currentDoctorDropdown.waitFor({ state: 'visible', timeout: 5000 });
  324 |             await currentDoctorDropdown.selectOption(doctor.value);
  325 |             
  326 |             // Wait for navigation to complete
  327 |             await currentPage.waitForLoadState('domcontentloaded', { timeout: 15000 });
  328 |             await currentPage.waitForLoadState('networkidle', { timeout: 15000 });
  329 |         } catch (e) {
  330 |             console.log(`[Visit] Navigation occurred for "${doctor.name}"`);
  331 |             // Page navigated - wait for it to settle
  332 |             await this.page.waitForTimeout(2000);
  333 |         }
  334 | 
  335 |         // Re-get page after potential navigation
  336 |         const activePage = this.page.context().pages().slice(-1)[0];
  337 |         
  338 |         // Check Visit Type on the NEW page context
  339 |         const visitTypeDropdown = activePage.locator('select[ng-model="Visit.VisitType"]').first();
  340 |         
  341 |         let hasNewVisitType = false;
  342 |         try {
  343 |             // Wait max 5s for options to appear
  344 |             await expect.poll(
  345 |                 async () => visitTypeDropdown.locator('option').count(),
  346 |                 { timeout: 5000 }
  347 |             ).toBeGreaterThan(1);
  348 | 
  349 |             const types = await visitTypeDropdown.locator('option').allInnerTexts();
  350 |             hasNewVisitType = types.map(t => t.trim()).includes(targetVisitType);
  351 |         } catch {
  352 |             console.log(`❌ [Visit] Visit Type empty for "${doctor.name}"`);
  353 |             continue; // try next doctor
  354 |         }
  355 | 
  356 |         if (hasNewVisitType) {
  357 |             console.log(`✅ [Visit] Found it! "${doctor.name}" supports "${targetVisitType}"`);
  358 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  359 |             foundWorkingDoctor = true;
  360 |             break; // STOP LOOP HERE
  361 |         }
  362 | 
  363 |         console.log(`❌ [Visit] "${doctor.name}" doesn't have "${targetVisitType}"`);
  364 |     }
  365 | 
  366 |     if (!foundWorkingDoctor) {
  367 |         throw new Error(`CRITICAL: No doctor found that supports Visit Type "${targetVisitType}".`);
  368 |     }
  369 | 
  370 |     // 4. Fill rest on the final page
  371 |     const finalPage = this.page.context().pages().slice(-1)[0];
  372 |     
  373 |     const queueInput = finalPage.locator('input[ng-model="Visit.TokenNo"]');
  374 |     await queueInput.fill(Math.floor(100 + Math.random() * 900).toString());
  375 | 
  376 |     const sourceDropdown = finalPage.locator('select[ng-model="Visit.PatientSourceID"]');
  377 |     await sourceDropdown.selectOption({ label: admissionSource });
  378 | 
  379 |     console.log('[Visit] fillVisitInformation complete ✅');
  380 | }
  381 | 
  382 | }
```