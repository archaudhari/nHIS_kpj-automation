...
```
Error: [Visit] No doctors loaded in Primary Doctor dropdown
```
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 22/05/2026 01:34:43"
            - listitem [ref=e13]:
...
```ts
  ...
  358 |     await this.page.waitForTimeout(2500);
  359 | 
  360 |     // ─────────────────────────────────────────────────────────────
  361 |     // 2. PRIMARY DOCTOR DROPDOWN
  362 |     // ─────────────────────────────────────────────────────────────
  363 | // ─────────────────────────────────────────────────────────────
  364 | // PRIMARY DOCTOR DROPDOWN
  365 | // ─────────────────────────────────────────────────────────────
  366 | const doctorDropdown = this.page.locator('#DoctorModality');
  367 | 
  368 | // Wait until dropdown attached
  369 | await doctorDropdown.waitFor({
  370 |     state: 'attached',
  371 |     timeout: 20000
  372 | });
  373 | 
  374 | // Wait until visible + enabled
  375 | await expect.poll(
  376 | 
  377 |     async () => {
  378 | 
  379 |         return await doctorDropdown.evaluate(
  380 |             (el: HTMLSelectElement) => {
  381 | 
  ...
  387 | 
  388 |                 const enabled = !el.disabled;
  389 | 
  390 |                 return visible && enabled;
  391 |             }
  392 |         );
  393 | 
  394 |     },
  ...
  401 | 
  402 | // Get doctors
  403 | const availableDoctors = await doctorDropdown.evaluate(
  404 |     (sel: HTMLSelectElement) => {
  405 | 
  406 |         return Array.from(sel.options)
  407 |             .map(o => ({
  408 |                 text: (o.text || '').trim(),
  409 |                 value: (o.value || '').trim()
  410 |             }))
  411 |             .filter(o => {
  412 | 
  413 |                 const text = o.text.toLowerCase();
  414 | 
  415 |                 return (
  416 |                     text !== '' &&
  417 |                     o.value !== '' &&
  418 |                     !text.includes('--select--') &&
  419 |                     !text.includes('loading')
  420 |                 );
  421 | 
  422 |             });
  423 | 
  ...
  426 | 
  427 | console.log(
  428 |     '[Visit] Available Doctors:',
  429 |     JSON.stringify(availableDoctors, null, 2)
  430 | );
  431 | 
  432 | if (availableDoctors.length === 0) {
  433 | 
> 434 |     throw new Error(
      |           ^ Error: [Visit] No doctors loaded in Primary Doctor dropdown
  435 |         '[Visit] No doctors loaded in Primary Doctor dropdown'
  436 |     );
  437 | }
  438 | //     const doctorDropdown = this.page.locator('#DoctorModality');
  439 | 
  440 | // // Wait until dropdown exists
  441 | // await doctorDropdown.waitFor({
  442 | //     state: 'attached',
  443 | //     timeout: 20000
  444 | // });
  445 | 
  446 | // // Wait until Angular removes hidden/disabled state
  447 | // await expect.poll(
  448 | 
  449 | //     async () => {
  450 | 
  451 | //         return await doctorDropdown.evaluate(
  452 | //             (el: HTMLSelectElement) => {
  453 | 
  454 | //                 const style = window.getComputedStyle(el);
  455 | 
  456 | //                 const visible =
  457 | //                     style.display !== 'none' &&
  458 | //                     style.visibility !== 'hidden';
  459 | 
  460 | //                 const enabled = !el.disabled;
  461 | 
  462 | //                 return visible && enabled;
  463 | //             }
  464 | //         );
  465 | 
  466 | //     },
  ...
  468 | //         timeout: 30000,
  469 | //         intervals: [1000],
  470 | //         message: 'Primary Doctor dropdown never became active'
  471 | //     }
  472 | 
  473 | // ).toBe(true);
  474 | 
  475 | //     // Wait for doctor list to load from backend
  476 | //     await expect.poll(
  477 | //         async () => {
  478 | 
  479 | //             return await doctorDropdown.evaluate(
  480 | //                 (sel: HTMLSelectElement) => {
  481 | 
  482 | //                     const validOptions = Array.from(sel.options)
  483 | //                         .filter(o => {
  484 | 
  485 | //                             const text = o.text
  486 | //                                 .trim()
  487 | //                                 .toLowerCase();
  488 | 
  489 | //                             return (
  490 | //                                 o.value !== '' &&
  491 | //                                 !text.includes('select') &&
  492 | //                                 !text.includes('loading')
  493 | //                             );
  494 | //                         });
  495 | 
  496 | //                     return validOptions.length;
  497 | //                 }
  498 | //             );
  499 | //         },
  500 | //         {
  501 | //             timeout: 20000,
  502 | //             intervals: [1000],
  503 | //             message: 'Doctor dropdown never populated'
  504 | //         }
  505 | //     ).toBeGreaterThan(0);
  506 | 
  507 | //     // Snapshot all available doctors
  508 | //     const availableDoctors = await doctorDropdown.evaluate(
  509 | //         (sel: HTMLSelectElement) => {
  510 | 
  511 | //             return Array.from(sel.options)
  512 | //                 .map(o => ({
  513 | //                     text: o.text.trim(),
  514 | //                     value: o.value
  515 | //                 }))
  516 | //                 .filter(o => {
  517 | 
  518 | //                     const text = o.text.toLowerCase();
  519 | 
  520 | //                     return (
  521 | //                         o.value !== '' &&
  522 | //                         !text.includes('select') &&
  523 | //                         !text.includes('loading')
  524 | //                     );
  525 | //                 });
  526 | //         }
  527 | //     );
  528 | 
  529 | //     console.log(
  530 | //         `[Visit] ${availableDoctors.length} doctors available`
  531 | //     );
  532 | 
  533 | //     if (availableDoctors.length === 0) {
  534 | //         throw new Error(
```