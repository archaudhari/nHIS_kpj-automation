      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 12:38:36"
            - listitem [ref=e13]:
...
```ts
  ...
  298 | 
  299 |     // 2. Get all doctor values BEFORE loop
  300 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  301 |     await expect.poll(
  302 |         async () => doctorDropdown.locator('option').count(),
  303 |         { timeout: 8000 }
  304 |     ).toBeGreaterThan(1);
  305 | 
  306 |     const options = await doctorDropdown.locator('option').all();
  307 |     const availableDoctors: { value: string, name: string }[] = [];
  308 |     for (let i = 1; i < options.length; i++) {
  309 |         const value = await options[i].getAttribute('value');
  310 |         const name = await options[i].innerText();
  311 |         if (value) availableDoctors.push({ value, name: name.trim() });
  312 |     }
  313 | 
  314 |     let selectedDoctorValue = '';
  315 | 
  316 |     // 3. Loop to find doctor with targetVisitType
  317 |     for (const doctor of availableDoctors) {
  318 |         console.log(`[Visit] Trying doctor: "${doctor.name}"...`);
  319 | 
  320 |         const currentDoctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  321 |         await currentDoctorDropdown.waitFor({ state: 'visible' });
  322 | 
  323 |         // Select doctor - may cause page reload
  324 |         try {
  325 |             await Promise.all([
  326 |                 this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {}),
  327 |                 currentDoctorDropdown.selectOption(doctor.value)
  328 |             ]);
  329 |         } catch {
  330 |             await this.page.waitForURL('**/VisitScreen*', { timeout: 15000 });
  331 |             await this.page.waitForLoadState('domcontentloaded');
  332 |             await this.page.waitForLoadState('networkidle');
  333 |         }
  334 | 
  335 |         // 4. Check if Department auto-loaded - use try/catch, not.then()
  336 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  337 |         let deptLoaded = false;
  338 |         try {
  339 |             await expect.poll(
  340 |                 async () => {
  341 |                     const val = await departmentDropdown.inputValue();
  342 |                     return val!== '' && val!== '0';
  343 |                 },
  344 |                 { timeout: 8000 }
  345 |             ).toBe(true);
  346 |             deptLoaded = true;
  347 |         } catch {
  348 |             deptLoaded = false;
  349 |         }
  350 | 
  351 |         if (!deptLoaded) {
  352 |             console.log(`[Visit] ⚠ Department did not auto-load for "${doctor.name}"`);
  353 |             continue;
  354 |         }
  355 | 
  356 |         // 5. Check Visit Type - get the count without.toBe().then()
  357 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  358 | 
  359 | // Wait for dropdown to populate with real options
> 360 | await expect.poll(
  ...
  364 | 
  365 | // Now just check if targetVisitType exists
  366 | const types = await visitTypeDropdown.locator('option').allInnerTexts();
  367 | const hasNewVisitType = types.map(t => t.trim()).includes(targetVisitType);
  368 | 
  369 | if (hasNewVisitType) {
  370 |     console.log(`✅ [Visit] Found it! "${doctor.name}" supports "${targetVisitType}"`);
  371 |     await visitTypeDropdown.selectOption({ label: targetVisitType });
  372 |     selectedDoctorValue = doctor.value;
  373 |     break; // >>>> Stop There
  374 | }
  375 | 
  376 | console.log(`❌ [Visit] "${doctor.name}" doesn't have "${targetVisitType}"`);
  377 |     }
  378 | 
  379 |     if (!selectedDoctorValue) {
  380 |         throw new Error(`CRITICAL: No doctor found that supports Visit Type "${targetVisitType}".`);
  381 |     }
  382 | 
  383 |     // 6. Fill remaining fields after loop
  384 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  385 |     await expect(queueInput).toBeEnabled({ timeout: 5000 });
  386 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  387 |     await queueInput.fill(randomQueue);
  388 | 
  389 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  390 |     await sourceDropdown.selectOption({ label: admissionSource });
  391 | 
  392 |     console.log('[Visit] fillVisitInformation complete ✅');
  393 | }
  394 | 
  395 | }
```