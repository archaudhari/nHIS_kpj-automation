...
```
Error: ❌ CRITICAL FAILURE: "New" is missing for Doctor Doctor_Annie!
```
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 03:06:55"
            - listitem [ref=e13]:
...
```ts
  ...
  330 |         await this.page.waitForTimeout(2000);
  331 |         
  ...
  337 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  338 |         const allOptions = await visitTypeDropdown.locator('option').allInnerTexts();
  339 |         const cleanOptions = allOptions.map(text => text.trim());
  340 |         
  341 |         console.log(`[3] Checking Visit Type... Available: [${cleanOptions.join(', ')}]`);
  342 | 
  343 |         if (cleanOptions.includes(targetVisitType)) {
  344 |             console.log(`    ✅ Success! "${targetVisitType}" is available.`);
  345 |             
  346 |             // Find the index and select it safely
  347 |             const targetIndex = cleanOptions.indexOf(targetVisitType);
  348 |             await visitTypeDropdown.selectOption({ index: targetIndex }, { force: true });
  349 |             await visitTypeDropdown.dispatchEvent('change');
  350 |             
  351 |         } else {
  352 |             // THEN STOP THERE: If "New" is missing, we crash the test immediately
> 353 |             throw new Error(`❌ CRITICAL FAILURE: "${targetVisitType}" is missing for Doctor ${actualFoundName}!`);
      |                   ^ Error: ❌ CRITICAL FAILURE: "New" is missing for Doctor Doctor_Annie!
  354 |         }
  355 | 
  356 |         // 5. PROCEED WITH QUEUE NO.
  357 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  358 |         await queueInput.waitFor({ state: 'visible' });
  359 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  360 |         
  361 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  362 |         await queueInput.fill(randomQueue);
  363 |         
  364 |         await this.page.locator('body').click(); // Trigger blur
  365 |         await this.page.waitForTimeout(500);
  366 |         console.log(`[4] Entered Queue No: ${randomQueue}\n`);
  367 |         // 6. ADMISSION SOURCE (DYNAMIC RANDOMIZER)
  368 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  369 |         await sourceDropdown.waitFor({ state: 'visible' });
  370 |         
  371 |         // Count how many options are in the dropdown
  372 |         const sourceOptionsCount = await sourceDropdown.locator('option').count();
  373 |         
  374 |         // Generate a random number between 1 and the total number of options
  375 |         // (We start at 1 to skip the 0 index, which is the "--Select--" placeholder)
  376 |         const randomSourceIndex = Math.floor(Math.random() * (sourceOptionsCount - 1)) + 1;
  377 | 
  378 |         // Select the random option by its Index position
  379 |         await sourceDropdown.selectOption({ index: randomSourceIndex }, { force: true });
  380 |         
  381 |         // CRITICAL: Fire the ng-change="PatientSourceChange()" event
  382 |         await sourceDropdown.dispatchEvent('change');
  383 |         await this.page.waitForTimeout(500);
  384 |         
  385 |         // Read the text of what was just selected so we can print it to the console
  386 |         const selectedSourceText = await sourceDropdown.locator('option').nth(randomSourceIndex).innerText();
  387 |         console.log(`[5] Selected Random Admission Source: ${selectedSourceText.trim()}\n`);
  388 |     }
  389 | }
```