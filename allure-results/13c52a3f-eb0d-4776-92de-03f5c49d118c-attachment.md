...
Call log:
  ...
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    132 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 02:31:46"
            - listitem [ref=e13]:
...
```ts
  ...
  294 | 
  295 |     // Target doctors - will be shuffled each run
  296 |     const doctors = [
  297 |         'Anaesthesiology & Critical Care - Doctor 12',
  298 |         'Radiology - Doctor 52', 
  299 |         'Internal Medicine - Doctor_Annie',
  300 |         'Cardiothoracic Surgery - Doctor 81',
  301 |         'Anatomical Pathology - Nor Liza Ariffin'
  302 |     ];
  303 | 
  304 |     // Shuffle array - Fisher-Yates
  305 |     const targetDoctors = [...doctors];
  306 |     for (let i = targetDoctors.length - 1; i > 0; i--) {
  307 |         const j = Math.floor(Math.random() * (i + 1));
  308 |         [targetDoctors[i], targetDoctors[j]] = [targetDoctors[j], targetDoctors[i]];
  309 |     }
  310 |     console.log(`[Visit] Randomized order: ${targetDoctors.join(' → ')}`);
  311 | 
  312 |     // 1. ENCOUNTER TYPE
  313 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  314 |     await encounterDropdown.waitFor({ state: 'visible' });
  315 |     await encounterDropdown.selectOption({ label: encounterType });
  316 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  317 |     console.log('[Visit] 1. Encounter Type set');
  318 | 
  319 |     // 2. PRIMARY DOCTOR first → use first doctor from shuffled list
  320 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  321 |     await expect.poll(async () => doctorDropdown.locator('option').count()).toBeGreaterThan(1);
  322 |     
  323 |     const primaryDoctor = targetDoctors[0];
> 324 |     await doctorDropdown.selectOption({ label: primaryDoctor });
      |                          ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  325 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  326 |     console.log(`[Visit] 2. Primary Doctor set: ${primaryDoctor}`);
  327 | 
  328 |     // 3. Confirm Department loaded
  329 |     const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  330 |     await expect.poll(async () => departmentDropdown.inputValue(), { timeout: 5000 }).not.toBe('0');
  331 |     const deptText = await departmentDropdown.locator('option:checked').innerText();
  332 |     console.log(`[Visit] 3. Department auto-loaded: ${deptText}`);
  333 | 
  334 |     // 4. Loop shuffled doctors until Visit Type "New" is available → select it → STOP
  335 |     let foundDoctor = '';
  336 |     
  337 |     for (let i = 0; i < targetDoctors.length; i++) {
  338 |         const doctorName = targetDoctors[i];
  339 |         console.log(`[Visit] 4.${i+1} Checking ${doctorName} for "${targetVisitType}"...`);
  340 | 
  341 |         const currentDoctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  342 |         await currentDoctorDropdown.selectOption({ label: doctorName });
  343 |         
  344 |         // Wait max 3s for Visit Type to populate
  345 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  346 |         
  347 |         const hasOptions = await this.page.waitForFunction(
  348 |             (sel) => {
  349 |                 const el = document.querySelector(sel) as HTMLSelectElement;
  350 |                 return el && el.options.length > 1;
  351 |             },
  352 |             'select[ng-model="Visit.VisitType"]',
  353 |             { timeout: 3000 }
  354 |         ).then(() => true).catch(() => false);
  355 | 
  356 |         if (!hasOptions) {
  357 |             console.log(`❌ [Visit] No Visit Types for ${doctorName}`);
  358 |             continue;
  359 |         }
  360 | 
  361 |         const types = await visitTypeDropdown.locator('option').allInnerTexts();
  362 |         const cleanTypes = types.map(t => t.trim()).filter(t => t && t!== '--Select--');
  363 | 
  364 |         if (cleanTypes.includes(targetVisitType)) {
  365 |             console.log(`✅ [Visit] FOUND! ${doctorName} supports "${targetVisitType}"`);
  366 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  367 |             foundDoctor = doctorName;
  368 |             break; // STOP LOOP
  369 |         }
  370 | 
  371 |         console.log(`❌ [Visit] ${doctorName} only has: [${cleanTypes.join(', ')}]`);
  372 |     }
  373 | 
  374 |     if (!foundDoctor) {
  375 |         throw new Error(`CRITICAL: None of the 5 doctors support "${targetVisitType}"`);
  376 |     }
  377 | 
  378 |     // 5. Queue No. - Random 3 digit
  379 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  380 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  381 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  382 |     await queueInput.fill(randomQueue);
  383 |     console.log(`[Visit] 5. Queue No: ${randomQueue}`);
  384 | 
  385 |     // 6. Admission Source
  386 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  387 |     await sourceDropdown.waitFor({ state: 'visible', timeout: 3000 });
  388 |     await sourceDropdown.selectOption({ label: admissionSource });
  389 |     console.log(`[Visit] 6. Admission Source: ${admissionSource}`);
  390 | 
  391 |     console.log(`[Visit] Complete in ${Date.now() - startTime}ms ✅ Final Doctor: ${foundDoctor}`);
  392 | }
  393 | 
  394 | }
```