...
Call log:
  - waiting for locator('select[ng-model="Visit.EncounterTypeID"]')
    - locator resolved to <select ng-model="Visit.EncounterTypeID" ng-options="item.value as item.text for item in drpEncounterType" class="form-control input-sm validate[required] ng-pristine ng-untouched ng-valid">…</select>
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    123 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 01:19:11"
            - listitem [ref=e13]:
...
```ts
  ...
  315 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  316 |     await encounterDropdown.waitFor({ state: 'visible', timeout: 5000 });
> 317 |     await encounterDropdown.selectOption({ label: encounterType }); // Will select "Outpatient"
      |                             ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  318 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  319 |     console.log(`[Visit] 1. Encounter Type set: ${encounterType}`);
  320 | 
  321 |     let foundDoctor = '';
  322 | 
  323 |     // 2-4. Loop: Doctor → Department → Visit Type New
  324 |     for (let i = 0; i < targetDoctors.length; i++) {
  325 |         const doctorName = targetDoctors[i];
  326 |         
  327 |         // 2. PRIMARY DOCTOR
  328 |         console.log(`[Visit] 2.${i+1} Selecting Doctor: ${doctorName}`);
  329 |         const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  330 |         await doctorDropdown.selectOption({ label: doctorName });
  331 |         await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  332 | 
  333 |         // 3. DEPARTMENT - Wait for auto-load
  334 |         console.log(`[Visit] 3.${i+1} Waiting for Department...`);
  335 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]');
  336 |         
  337 |         const deptLoaded = await this.page.waitForFunction(
  338 |             (sel: string) => {
  339 |                 const el = document.querySelector(sel) as HTMLSelectElement | null;
  340 |                 return el && el.value && el.value!== '0' && el.value!== '';
  341 |             },
  342 |             'select[ng-model="Visit.DepartmentID"]',
  343 |             { timeout: 3000 }
  344 |         ).then(() => true).catch(() => false);
  345 | 
  346 |         if (!deptLoaded) {
  347 |             console.log(`❌ Department failed to load for ${doctorName}`);
  348 |             continue;
  349 |         }
  350 | 
  351 |         const deptText = await departmentDropdown.locator('option:checked').innerText();
  352 |         console.log(`✓ Department loaded: ${deptText}`);
  353 | 
  354 |         // 4. VISIT TYPE - Check for "New"
  355 |         console.log(`[Visit] 4.${i+1} Checking Visit Type "New"...`);
  356 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  357 |         
  358 |         const hasNew = await this.page.waitForFunction(
  359 |             (args: { selector: string; targetType: string }) => {
  360 |                 const el = document.querySelector(args.selector) as HTMLSelectElement | null;
  361 |                 if (!el || el.options.length <= 1) return false;
  362 |                 return Array.from(el.options).some(opt => opt.text.trim() === args.targetType);
  363 |             },
  364 |             { selector: 'select[ng-model="Visit.VisitType"]', targetType: targetVisitType },
  365 |             { timeout: 3000 }
  366 |         ).then(() => true).catch(() => false);
  367 | 
  368 |         if (hasNew) {
  369 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  370 |             foundDoctor = doctorName;
  371 |             console.log(`✅ SUCCESS: ${doctorName} → ${deptText} → "${targetVisitType}"`);
  372 |             break;
  373 |         }
  374 | 
  375 |         console.log(`❌ "${targetVisitType}" not available for ${doctorName}`);
  376 |     }
  377 | 
  378 |     if (!foundDoctor) {
  379 |         throw new Error(`No doctor has Department + "${targetVisitType}"`);
  380 |     }
  381 | 
  382 |     // 5. Queue No.
  383 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  384 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  385 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  386 |     await queueInput.fill(randomQueue);
  387 |     console.log(`[Visit] 5. Queue No: ${randomQueue}`);
  388 | 
  389 |     // 6. Admission Source
  390 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  391 |     await sourceDropdown.selectOption({ label: admissionSource });
  392 |     console.log(`[Visit] 6. Admission Source: ${admissionSource}`);
  393 | 
  394 |     console.log(`[Visit] Complete in ${Date.now() - startTime}ms ✅ Doctor: ${foundDoctor}`);
  395 | }
  396 | }
```