...
```
Error: CRITICAL: No doctor found that supports Visit Type "New".
```
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 12:28:53"
            - listitem [ref=e13]:
              ...
              - generic [ref=e102]:
                ...
                - generic [ref=e129]:
                  ...
                  - combobox [ref=e132]:
                    ...
                    - option "Resident Medical Officer" [selected]
...
```ts
  ...
  292 | ) {
  293 |     // 1. ENCOUNTER TYPE - only do this once
  294 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  295 |     await encounterDropdown.waitFor({ state: 'visible' });
  296 |     await encounterDropdown.selectOption({ label: encounterType });
  297 | 
  298 |     await this.page.waitForLoadState('networkidle');
  299 |     console.log(`[Visit] Encounter Type set → "${encounterType}"`);
  300 | 
  301 |     // 2. Get all doctor values ONCE, before any selections
  302 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  303 |     await expect.poll(
  304 |         async () => doctorDropdown.locator('option').count(),
  305 |         { timeout: 8000 }
  306 |     ).toBeGreaterThan(1);
  307 | 
  308 |     const doctorOptionsCount = await doctorDropdown.locator('option').count();
  309 |     const doctorValues: string[] = [];
  310 |     for (let i = 1; i < doctorOptionsCount; i++) {
  311 |         const val = await doctorDropdown.locator('option').nth(i).getAttribute('value');
  312 |         if (val) doctorValues.push(val);
  313 |     }
  314 |     console.log(`[Visit] ${doctorValues.length} doctor(s) to check for "${targetVisitType}"...`);
  315 | 
  316 |     // 3. Try each doctor. If page reloads, we'll handle it
  317 |     for (let i = 0; i < doctorValues.length; i++) {
  318 |         const doctorValue = doctorValues[i];
  319 | 
  320 |         try {
  321 |             // Always re-locate after potential reload from previous iteration
  322 |             const currentDoctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  323 |             await currentDoctorDropdown.waitFor({ state: 'visible', timeout: 5000 });
  324 | 
  325 |             const doctorName = await currentDoctorDropdown.locator(`option[value="${doctorValue}"]`).innerText();
  326 |             console.log(`[Visit] Trying doctor: "${doctorName.trim()}"...`);
  327 | 
  328 |             // This might trigger a full page reload
  329 |             await currentDoctorDropdown.selectOption(doctorValue);
  330 | 
  331 |             // Wait for Angular to finish whatever it does - reload or AJAX
  332 |             await this.page.waitForLoadState('domcontentloaded', { timeout: 10000 });
  333 |             await this.page.waitForLoadState('networkidle', { timeout: 10000 });
  334 | 
  335 |         } catch (e) {
  336 |             // If page closed/reloaded mid-selectOption, wait for it to come back
  337 |             console.log(`[Visit] Page reloaded during doctor select. Waiting...`);
  338 |             await this.page.waitForURL('**/VisitScreen*', { timeout: 15000 });
  339 |             await this.page.waitForLoadState('domcontentloaded');
  340 |             await this.page.waitForLoadState('networkidle');
  341 |         }
  342 | 
  343 |         // 4. Now check if this doctor worked - re-query everything
  344 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  345 |         const deptValue = await departmentDropdown.inputValue().catch(() => '');
  346 | 
  347 |         if (deptValue === '' || deptValue === '0') {
  348 |             console.log(`[Visit] ⚠ Department did not auto-load. Trying next doctor...`);
  349 |             continue;
  350 |         }
  351 | 
  352 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  353 |         const visitTypeCount = await visitTypeDropdown.locator('option').count().catch(() => 0);
  354 | 
  355 |         if (visitTypeCount <= 1) {
  356 |             console.log(`[Visit] Visit Type empty. Trying next doctor...`);
  357 |             continue;
  358 |         }
  359 | 
  360 |         const visitTypes = (await visitTypeDropdown.locator('option').allInnerTexts()).map(t => t.trim());
  361 | 
  362 |         if (visitTypes.includes(targetVisitType)) {
  363 |             console.log(`✅ [Visit] Found working doctor. Selecting "${targetVisitType}" & stopping.`);
  364 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  365 |             await this.page.waitForLoadState('networkidle');
  366 | 
  367 |             // 5. Continue with Queue No + Admission Source
  368 |             const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  369 |             await expect(queueInput).toBeEnabled({ timeout: 5000 });
  370 |             const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  371 |             await queueInput.fill(randomQueue);
  372 | 
  373 |             const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  374 |             await sourceDropdown.selectOption({ label: admissionSource });
  375 | 
  376 |             console.log('[Visit] fillVisitInformation complete ✅');
  377 |             return; // >>>> Then Stop There
  378 |         }
  379 | 
  380 |         console.log(`❌ [Visit] "${targetVisitType}" not available. Trying next...`);
  381 |     }
  382 | 
> 383 |     throw new Error(`CRITICAL: No doctor found that supports Visit Type "${targetVisitType}".`);
      |           ^ Error: CRITICAL: No doctor found that supports Visit Type "New".
  384 | }
  385 | 
  386 | }
