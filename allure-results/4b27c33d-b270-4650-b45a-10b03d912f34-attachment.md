...
```
Error: locator.click: Error: strict mode violation: locator('button:has-text("Save"), input[value="Save"]') resolved to 4 elements:
...
Call log:
  - waiting for locator('button:has-text("Save"), input[value="Save"]')
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 23/05/2026 11:58:29"
            - listitem [ref=e13]:
...
```ts
  ...
  519 |         // ─────────────────────────────────────────────────────────────
  520 | // SAVE REGISTRATION
  521 | // ─────────────────────────────────────────────────────────────
  522 | 
  523 | // Save button
  524 | const saveButton = this.page.locator(
  525 |     'button:has-text("Save"), input[value="Save"]'
  526 | );
  527 | 
  528 | // Click Save
> 529 | await saveButton.click();
  ...
  530 | 
  531 | console.log(
  532 |     '[Registration] Save button clicked'
  533 | );
  534 | 
  535 | // ─────────────────────────────────────────────────────────────
  536 | // CONFIRMATION POPUP
  537 | // ─────────────────────────────────────────────────────────────
  538 | 
  539 | // Wait for confirmation popup
  540 | const confirmationPopup = this.page.locator(
  541 |     '.ng-confirm-content-pane'
  542 | );
  543 | 
  544 | await confirmationPopup.waitFor({
  545 |     state: 'visible',
  546 |     timeout: 15000
  547 | });
  548 | 
  549 | // Validate popup text
  550 | await expect(
  551 |     confirmationPopup
  552 | ).toContainText(
  553 |     'Do you want to Save'
  554 | );
  555 | 
  556 | console.log(
  557 |     '[Registration] Save confirmation popup appeared'
  558 | );
  559 | 
  560 | // ─────────────────────────────────────────────────────────────
  561 | // CONFIRM SAVE
  562 | // ─────────────────────────────────────────────────────────────
  563 | 
  564 | // Click popup Save button
  565 | const confirmSaveButton = this.page.locator(
  566 |     'span.ng-confirm-btn-text'
  567 | ).filter({
  568 |     hasText: 'Save'
  569 | });
  570 | 
  571 | await confirmSaveButton.click();
  572 | 
  573 | console.log(
  574 |     '[Registration] Confirm Save clicked'
  575 | );
  576 | 
  577 | // Wait for save completion
  578 | await this.page.waitForLoadState('networkidle');
  579 | await this.page.waitForTimeout(5000);
  580 |     }
  581 | 
  582 | }
  583 | 
