      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 18/05/2026 01:40:09"
            - listitem [ref=e13]:
...
```ts
  ...
  254 |         // 1. Look for a field that we know lives INSIDE the Visit Information tab
  255 |         const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  256 |         
  257 |         // 2. Check if that field is already visible on the screen
  258 |         // (We use a short timeout so it doesn't wait 30 seconds to figure this out)
  259 |         const isAlreadyOpen = await encounterDropdown.isVisible({ timeout: 1000 });
  260 | 
  261 |         if (!isAlreadyOpen) {
  262 |             console.log("Visit Information is closed. Clicking the tab to open it...");
  263 |             
  264 |             // Try to click the specific header. You may need to change 'text=' 
  265 |             // to a specific CSS class if there are multiple "Visit Information" texts on the screen.
  266 |             const visitInfoTab = this.page.locator('text="Visit Information"').first();
  267 |             await visitInfoTab.waitFor({ state: 'visible' });
  268 |             await visitInfoTab.click();
  269 |             
  270 |             // Wait a moment for the sliding animation to finish
  271 |             await this.page.waitForTimeout(500); 
  272 |         } else {
  273 |             console.log("Visit Information is already open! Skipping the click.");
  274 |         }
  275 |     }
  276 | 
  277 |     async fillVisitInformation(encounterType: string, department: string, doctor: string, visitType: string, admissionSource: string) {
  278 |         
  279 |         const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  280 |         await encounterDropdown.waitFor({ state: 'visible' });
  281 |         await encounterDropdown.selectOption({ label: encounterType });
  282 |         await this.page.waitForTimeout(500);
  283 | 
  284 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]');
  285 |         await departmentDropdown.waitFor({ state: 'visible' });
  286 |         await departmentDropdown.selectOption({ label: department });
  287 |         await departmentDropdown.dispatchEvent('change');
  288 |         await this.page.waitForTimeout(1000); 
  289 | 
  290 |         // 3. Primary Doctor
  291 |         const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  292 |         await doctorDropdown.waitFor({ state: 'visible' });
  293 | 
  294 |         // WAIT for the application to fetch the doctors from the backend
  295 |         // (Wait up to 5 seconds for at least 2 options to appear in the list)
  296 |         await doctorDropdown.locator('option').nth(1).waitFor({ state: 'attached', timeout: 5000 });
  297 | 
  298 |         // ==========================================================
  299 |         // 🕵️ DEBUGGING MAGIC: Print all available doctors to the console
  300 |         // ==========================================================
  301 |         const availableDoctors = await doctorDropdown.locator('option').allInnerTexts();
  302 |         console.log(`\n--- DOCTORS AVAILABLE FOR DEPARTMENT: ${department} ---`);
  303 |         console.log(availableDoctors);
  304 |         console.log(`------------------------------------------------------\n`);
  305 | 
  306 |         try {
  307 |             // Attempt to select the doctor
> 308 |             await doctorDropdown.selectOption({ label: doctor }, { force: true });
      |                                  ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  309 |         } catch (error) {
  310 |             console.error(`❌ CRITICAL FAILURE: Could not find "${doctor}" in the list above!`);
  311 |             console.error(`Check the console output above to see exactly what strings Playwright is seeing.`);
  312 |             throw error; // Fail the test so you can fix your data
  313 |         }
  314 | 
  315 |         await doctorDropdown.dispatchEvent('change');
  316 |         await this.page.waitForTimeout(1000);
  317 | 
  318 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  319 |         await visitTypeDropdown.waitFor({ state: 'visible' });
  320 |         await visitTypeDropdown.selectOption({ label: visitType }, { force: true });
  321 |         await visitTypeDropdown.dispatchEvent('change');
  322 |         await this.page.waitForTimeout(500)
  323 | 
  324 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  325 |         await sourceDropdown.waitFor({ state: 'visible' });
  326 |         await sourceDropdown.selectOption({ label: admissionSource });
  327 |         await sourceDropdown.dispatchEvent('change');
  328 |         await this.page.waitForTimeout(500);
  329 | 
  330 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  331 |         
  332 |         // Wait for it to be enabled (Usually, HIS systems enable this AFTER a doctor is selected)
  333 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  334 |         
  335 |         // Generate a random 3-digit queue number and fill it
  336 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  337 |         await queueInput.fill(randomQueue);
  338 |         
  339 |         // Trigger a blur event just in case Angular validates the queue number against the doctor's active queue
  340 |         await this.page.locator('body').click();
