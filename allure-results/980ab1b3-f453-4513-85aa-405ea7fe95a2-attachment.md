...
Call log:
  ...
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    125 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
...
- generic [active] [ref=e1]:
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 01:39:03"
            - listitem [ref=e13]:
  ...
  - generic [ref=e274]: OPD-IPD Counter
  - generic [ref=e275]: 72%
...
> 305 |         await encounterDropdown.selectOption({ label: encounterType });
      |                                 ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  306 |         await encounterDropdown.dispatchEvent('change');
  307 |         console.log(`[Visit] 1. Encounter Type → "${encounterType}"`);
  308 |         await this.page.waitForTimeout(1000); // let Angular digest + render doctor dropdown
  309 | 
  310 |         // ── 2. FIND THE REAL DOCTOR DROPDOWN ───────────────────────────
  311 |         // The ng-model name was unknown/guessed. We probe all visible
  312 |         // <select> elements in the Visit panel and pick the one whose
  313 |         // ng-model contains "Doctor" or "doctor" (case-insensitive).
  314 |         const doctorNgModel: string = await this.page.evaluate(() => {
  315 |             const selects = Array.from(document.querySelectorAll('select[ng-model]'));
  316 |             const match = selects.find(s => {
  317 |                 const model = s.getAttribute('ng-model') ?? '';
  318 |                 return model.toLowerCase().includes('doctor');
  319 |             });
  320 |             return match?.getAttribute('ng-model') ?? '';
  321 |         });
  322 | 
  323 |         if (!doctorNgModel) {
  324 |             throw new Error(
  325 |                 '[Visit] Could not find any <select ng-model="*doctor*"> in the DOM. ' +
  326 |                 'Ensure Encounter Type selection renders the doctor dropdown before this step.'
  327 |             );
  328 |         }
  329 |         console.log(`[Visit] 2. Doctor dropdown found → ng-model="${doctorNgModel}"`);
  330 | 
  331 |         // Wait until that dropdown has at least one real option
  332 |         await expect.poll(
  333 |             () => this.page.evaluate((model: string) => {
  334 |                 const sel = document.querySelector(
  335 |                     `select[ng-model="${model}"]`
  336 |                 ) as HTMLSelectElement | null;
  337 |                 return sel ? sel.options.length : 0;
  338 |             }, doctorNgModel),
  339 |             { timeout: 10000, message: `Doctor dropdown (${doctorNgModel}) never populated` }
  340 |         ).toBeGreaterThan(1);
  341 | 
  342 |         // Snapshot all non-placeholder doctor options into memory
  343 |         const availableDoctors: { text: string; value: string }[] =
  344 |             await this.page.evaluate((model: string) => {
  345 |                 const sel = document.querySelector(
  346 |                     `select[ng-model="${model}"]`
  347 |                 ) as HTMLSelectElement | null;
  348 |                 if (!sel) return [];
  349 |                 return Array.from(sel.options)
  350 |                     .map(o => ({ text: o.text.trim(), value: o.value }))
  351 |                     .filter(o =>
  352 |                         o.value !== '' &&
  353 |                         !o.value.startsWith('string:') &&  // AngularJS placeholder
  354 |                         !o.text.toLowerCase().includes('select')
  355 |                     );
  356 |             }, doctorNgModel);
  357 | 
  358 |         console.log(`[Visit] ${availableDoctors.length} doctor(s) available. Scanning for Visit Type "${targetVisitType}"...`);
  359 | 
  360 |         if (availableDoctors.length === 0) {
  361 |             throw new Error(`[Visit] No selectable doctor options found in dropdown (ng-model="${doctorNgModel}")`);
  362 |         }
  363 | 
  364 |         // ── 3. DOCTOR LOOP ──────────────────────────────────────────────
  365 |         // Everything inside this loop uses page.evaluate (runs atomically
  366 |         // in the browser) so stale-element errors cannot occur.
  367 |         let foundWorkingDoctor = false;
  368 | 
  369 |         for (const doctor of availableDoctors) {
  370 |             console.log(`[Visit] Trying doctor: "${doctor.text}"...`);
  371 | 
  372 |             // Set doctor value + dispatch Angular change — all in one browser call
  373 |             await this.page.evaluate(
  374 |                 ({ model, val }: { model: string; val: string }) => {
  375 |                     const sel = document.querySelector(
  376 |                         `select[ng-model="${model}"]`
  377 |                     ) as HTMLSelectElement | null;
  378 |                     if (!sel) throw new Error(`Doctor select not found: ${model}`);
  379 |                     sel.value = val;
  380 |                     sel.dispatchEvent(new Event('change', { bubbles: true }));
  381 |                 },
  382 |                 { model: doctorNgModel, val: doctor.value }
  383 |             );
  384 | 
  385 |             await this.page.waitForTimeout(800); // Angular digest: department auto-loads
  386 | 
  387 |             // ── 3a. Log which Department auto-loaded ─────────────────────
  388 |             const deptLabel: string = await this.page.evaluate(() => {
  389 |                 const sel = document.querySelector(
  390 |                     'select[ng-model="Visit.DepartmentID"]'
  391 |                 ) as HTMLSelectElement | null;
  392 |                 return sel?.options[sel.selectedIndex]?.text?.trim() ?? '(not found)';
  393 |             });
  394 |             console.log(`[Visit]   Department auto-loaded → "${deptLabel}"`);
  395 | 
  396 |             // ── 3b. Find Visit Type dropdown ng-model (probe once) ───────
  397 |             const visitTypeNgModel: string = await this.page.evaluate(() => {
  398 |                 const selects = Array.from(document.querySelectorAll('select[ng-model]'));
  399 |                 const match = selects.find(s => {
  400 |                     const m = s.getAttribute('ng-model') ?? '';
  401 |                     return m.toLowerCase().includes('visittype') || m.toLowerCase().includes('visit.type');
  402 |                 });
  403 |                 return match?.getAttribute('ng-model') ?? 'Visit.VisitType';
  404 |             });
  405 | 
```