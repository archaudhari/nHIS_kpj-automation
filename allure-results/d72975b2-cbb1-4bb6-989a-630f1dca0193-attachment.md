...
Call log:
  ...
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    135 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 02:42:49"
            - listitem [ref=e13]:
...
```ts
  ...
  294 | 
  295 |     // Declare targetDoctors here - with the 5 doctors you gave
  296 |     const targetDoctors = [
  ...
  302 |     ];
  303 | 
  304 |     // Shuffle for randomness each run
  305 |     for (let i = targetDoctors.length - 1; i > 0; i--) {
  306 |         const j = Math.floor(Math.random() * (i + 1));
  307 |         [targetDoctors[i], targetDoctors[j]] = [targetDoctors[j], targetDoctors[i]];
  308 |     }
  309 |     console.log(`[Visit] Test order: ${targetDoctors.join(' → ')}`);
  310 | 
  311 |     // 1. ENCOUNTER TYPE
  312 |     const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  313 |     await encounterDropdown.waitFor({ state: 'visible' });
  314 |     await encounterDropdown.selectOption({ label: encounterType });
  315 |     await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  316 |     console.log('[Visit] 1. Encounter Type set');
  317 | 
  318 |     // 2-4. Loop doctors: Select → Check Dept → Check "New" → STOP if both OK
  319 |     let foundDoctor = '';
  320 | 
  321 |     for (let i = 0; i < targetDoctors.length; i++) {
  322 |         const doctorName = targetDoctors[i];
  323 |         console.log(`[Visit] 4.${i+1} Checking ${doctorName} for "${targetVisitType}"...`);
  324 | 
...
> 326 |         await currentDoctorDropdown.selectOption({ label: doctorName });
  ...
  328 | 
  329 |         // Check Department loaded
  330 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  331 |         const deptLoaded = await this.page.waitForFunction(
  332 |             (sel: string) => {
  333 |                 const el = document.querySelector(sel) as HTMLSelectElement;
  334 |                 return el && el.value && el.value!== '0' && el.value!== '';
  335 |             },
  336 |             'select[ng-model="Visit.DepartmentID"]',
  337 |             { timeout: 3000 }
  338 |         ).then(() => true).catch(() => false);
  339 | 
  340 |         if (!deptLoaded) {
  341 |             console.log(`❌ Dept not loaded for ${doctorName}`);
  342 |             continue;
  343 |         }
  344 | 
  345 |         const deptText = await departmentDropdown.locator('option:checked').innerText();
  346 |         console.log(`✓ Dept loaded: ${deptText}`);
  347 | 
  348 |         // Check Visit Type "New"
  349 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  350 |         
  351 |         const hasNew = await this.page.waitForFunction(
  352 |             (args: { selector: string; targetType: string }) => {
  353 |                 const el = document.querySelector(args.selector) as HTMLSelectElement | null;
  354 |                 if (!el) return false;
  355 |                 return Array.from(el.options).some(opt => opt.text.trim() === args.targetType);
  356 |             },
  357 |             { selector: 'select[ng-model="Visit.VisitType"]', targetType: targetVisitType },
  358 |             { timeout: 3000 }
  359 |         ).then(() => true).catch(() => false);
  360 | 
  361 |         if (hasNew) {
  362 |             console.log(`✅ STOPPING: ${doctorName} has Dept + "${targetVisitType}"`);
  363 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  364 |             foundDoctor = doctorName;
  365 |             break; // STOP LOOP
  366 |         }
  367 | 
  368 |         console.log(`❌ ${doctorName} missing "${targetVisitType}"`);
  369 |     }
  370 | 
  371 |     if (!foundDoctor) {
  372 |         throw new Error(`CRITICAL: None of the doctors support "${targetVisitType}"`);
  373 |     }
  374 | 
  375 |     // 5. Queue No.
  376 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  377 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  378 |     await queueInput.fill(Math.floor(100 + Math.random() * 900).toString());
  379 | 
  380 |     // 6. Admission Source
  381 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  382 |     await sourceDropdown.selectOption({ label: admissionSource });
  383 | 
  384 |     console.log(`[Visit] Done in ${Date.now() - startTime}ms. Used: ${foundDoctor}`);
  385 | }
  386 | }
```