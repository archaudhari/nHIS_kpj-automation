...
```
Error: locator.waitFor: Error: strict mode violation: locator('button.btn.btn-primary').filter({ hasText: 'Save' }) resolved to 4 elements:
    1) <button type="button" ng-click="fnSaveAddDetails()" class="btn btn-primary btn-sm width-bigger-btn">Save</button> aka locator('#AddDetails').getByText('Save')
    2) <button type="button" id="regbutton" ng-click="IUDRegistration();" class="btn btn-primary btn-sm width-bigger-btn" ng-disabled="Visit.SaveBtnEnable==0|| addressLengthError">Save </button> aka getByRole('button', { name: 'Save' })
    3) <button type="button" ng-click="IUDconsentdetail()" class="btn btn-primary btn-sm">Save</button> aka locator('#visitScreenConsentModal button').filter({ hasText: 'Save' })
    4) <button type="button" onclick="CheckPasswd()" class="btn btn-primary btn-sm width-bigger-btn">Save</button> aka locator('#changePass').getByText('Save')

Call log:
  - waiting for locator('button.btn.btn-primary').filter({ hasText: 'Save' }) to be visible
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 24/05/2026 12:47:54"
            - listitem [ref=e13]:
...
```ts
  ...
  519 | 
  520 | // ─────────────────────────────────────────────────────────────
  521 | // SAVE REGISTRATION
  522 | // ─────────────────────────────────────────────────────────────
  523 | 
  524 | // Save button
  525 | const saveButton = this.page.locator(
  526 |     'button.btn.btn-primary'
  527 | ).filter({
  528 |     hasText: 'Save'
  529 | });
> 530 | await saveButton.waitFor({
      |                  ^ Error: locator.waitFor: Error: strict mode violation: locator('button.btn.btn-primary').filter({ hasText: 'Save' }) resolved to 4 elements:
  531 |     state: 'visible',
  532 |     timeout: 15000
  533 | });
  534 | 
  535 | // Click Save
  536 | await saveButton.click();
  537 | 
  538 | console.log(
  539 |     '[Registration] Save button clicked'
  540 | );
  541 | 
  542 | // ─────────────────────────────────────────────────────────────
  543 | // CONFIRMATION POPUP
  544 | // ─────────────────────────────────────────────────────────────
  545 | 
  546 | // Wait for confirmation popup
  547 | const confirmationPopup = this.page.locator(
  548 |     '.ng-confirm-content-pane'
  549 | );
  550 | 
  551 | await confirmationPopup.waitFor({
  552 |     state: 'visible',
  553 |     timeout: 15000
  554 | });
  555 | 
  556 | // Validate popup text
  557 | await expect(
  558 |     confirmationPopup
  559 | ).toContainText(
  560 |     'Do you want to Save'
  561 | );
  562 | 
  563 | console.log(
  564 |     '[Registration] Save confirmation popup appeared'
  565 | );
  566 | 
  567 | // ─────────────────────────────────────────────────────────────
  568 | // CONFIRM SAVE
  569 | // ─────────────────────────────────────────────────────────────
  570 | 
  571 | // Click popup Save button
  572 | const confirmSaveButton = this.page.locator(
  573 |     'span.ng-confirm-btn-text'
  574 | ).filter({
  575 |     hasText: 'Save'
  576 | });
  577 | 
  578 | await confirmSaveButton.click();
  579 | 
  580 | console.log(
  581 |     '[Registration] Confirm Save clicked'
  582 | );
  583 | 
  584 | // Wait for save completion
  585 | await this.page.waitForLoadState('networkidle');
  586 | await this.page.waitForTimeout(5000);
  587 |     }
  588 | 
  589 | }
  590 | 
