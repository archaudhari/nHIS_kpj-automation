...
Locator: locator('select[ng-model="Visit.VisitType"]').first()
Expected substring: "New"
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toContainText" with timeout 10000ms
  - waiting for locator('select[ng-model="Visit.VisitType"]').first()
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 03:33:42"
            - listitem [ref=e13]:
...
```ts
  ...
  290 |     // =====================================================================
  291 |     async fillVisitInformation(targetDoctorName: string, targetVisitType: string = 'New') {
  292 |         
  293 |         console.log(`\n--- Starting Visit Info for Doctor: ${targetDoctorName} ---`);
  294 | 
  295 |         // 1. EXTRACT DOCTORS & FIND TARGET
  296 |         const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  297 |         await doctorDropdown.waitFor({ state: 'visible' });
  298 |         await doctorDropdown.locator('option').nth(1).waitFor({ state: 'attached', timeout: 5000 });
  299 |         
  300 |         const doctorOptionsCount = await doctorDropdown.locator('option').count();
  301 |         let doctorValueToSelect = null;
  302 |         let actualFoundName = '';
  303 | 
  304 |         // Search the dropdown for the specific doctor you passed in
  305 |         for (let i = 1; i < doctorOptionsCount; i++) {
  306 |             const option = doctorDropdown.locator('option').nth(i);
  307 |             const name = await option.innerText();
  308 |             
  309 |             // Using .includes() bypasses any trailing space errors from the database!
  310 |             if (name.includes(targetDoctorName)) { 
  311 |                 doctorValueToSelect = await option.getAttribute('value');
  312 |                 actualFoundName = name.trim();
  313 |                 break; // Found our doctor, stop searching the list
  314 |             }
  315 |         }
  316 | 
  317 |         // Fail-safe if the doctor name you typed doesn't exist in the system
  318 |         if (!doctorValueToSelect) {
  319 |             throw new Error(`❌ CRITICAL FAILURE: Could not find "${targetDoctorName}" in the Doctor dropdown!`);
  320 |         }
  321 | 
  322 |         // 2. FIRST: SELECT PRIMARY DOCTOR
  323 |         console.log(`[1] Selecting Primary Doctor: ${actualFoundName}`);
  324 |         await doctorDropdown.selectOption({ value: doctorValueToSelect }, { force: true });
  325 |         await doctorDropdown.dispatchEvent('change');
  326 |         await doctorDropdown.dispatchEvent('blur');
  327 | 
  328 |         // 3. WAIT FOR DEPARTMENT TO AUTO-LOAD
  329 |         // We wait 2 seconds for Angular to fetch the Department and Visit Types
  330 |         await this.page.waitForTimeout(5000);
  331 |         
  332 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]');
  333 |         const autoLoadedDept = await departmentDropdown.locator('option:checked').innerText();
  334 |         console.log(`[2] Auto-Loaded Department: ${autoLoadedDept.trim()}`);
  335 | 
  336 |         // 4. CHECK VISIT TYPE: NEW
  337 |         // 1. Check if "New" loaded in the Visit Type dropdown
  338 |         // 4. CHECK VISIT TYPE: NEW
  339 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
  340 | 
  341 |         // try {
  342 |             // THE FIX: Playwright will now stare at the dropdown for up to 10 seconds.
  343 |             // The exact millisecond "New" appears, it will instantly move forward.
> 344 |             await expect(visitTypeDropdown).toContainText('New', { timeout: 10000 });
      |                                             ^ Error: expect(locator).toContainText(expected) failed
  345 |             
  346 |             console.log('✅ "New" Visit Type loaded. Selecting it...');
  347 |             
  348 |             // Because we verified it exists, we can safely select it by its label
  349 |             await visitTypeDropdown.selectOption({ label: 'New' }, { force: true });
  350 |             await visitTypeDropdown.dispatchEvent('change');
  351 | 
  352 |             const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  353 |         await queueInput.waitFor({ state: 'visible' });
  354 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  355 |         
  356 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  357 |         await queueInput.fill(randomQueue);
  358 |         
  359 |         await this.page.locator('body').click(); // Trigger blur
  360 |         await this.page.waitForTimeout(500);
  361 |         console.log(`[4] Entered Queue No: ${randomQueue}\n`);
  362 |         // 6. ADMISSION SOURCE (DYNAMIC RANDOMIZER)
  363 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  364 |         await sourceDropdown.waitFor({ state: 'visible' });
  365 |         
  366 |         // Count how many options are in the dropdown
  367 |         const sourceOptionsCount = await sourceDropdown.locator('option').count();
  368 |         
  369 |         // Generate a random number between 1 and the total number of options
  370 |         // (We start at 1 to skip the 0 index, which is the "--Select--" placeholder)
  371 |         const randomSourceIndex = Math.floor(Math.random() * (sourceOptionsCount - 1)) + 1;
  372 | 
  373 |         // Select the random option by its Index position
  374 |         await sourceDropdown.selectOption({ index: randomSourceIndex }, { force: true });
  375 |         
  376 |         // CRITICAL: Fire the ng-change="PatientSourceChange()" event
  377 |         await sourceDropdown.dispatchEvent('change');
  378 |         await this.page.waitForTimeout(500);
  379 |         
  380 |         // Read the text of what was just selected so we can print it to the console
  381 |         const selectedSourceText = await sourceDropdown.locator('option').nth(randomSourceIndex).innerText();
  382 |         console.log(`[5] Selected Random Admission Source: ${selectedSourceText.trim()}\n`);
  383 |             
  384 |         // } 
  385 |         // catch (error) {
  386 |         //     // Failsafe: Only throw this error if 10 full seconds pass and "New" STILL isn't there.
  387 |         //     throw new Error('❌ CRITICAL FAILURE: The "New" Visit Type did not load for this doctor within 10 seconds.');
  388 |         // }
  389 | 
  390 | 
  391 |         // 5. PROCEED WITH QUEUE NO.
  392 |         
  393 |     }
  394 | }
```