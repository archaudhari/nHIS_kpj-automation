      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 19/05/2026 01:12:26"
            - listitem [ref=e13]:
...
```ts
  ...
  299 |     await this.page.waitForLoadState('networkidle', { timeout: 5000 });
  300 |     console.log(`[Visit] Encounter set in ${Date.now() - startTime}ms`);
  301 | 
  302 |     // 2. Get all doctors
  303 |     const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  304 |     await expect.poll(
  305 |         async () => doctorDropdown.locator('option').count(),
  306 |         { timeout: 5000 }
  307 |     ).toBeGreaterThan(1);
  308 | 
  309 |     const options = await doctorDropdown.locator('option').all();
  310 |     const availableDoctors: { value: string, name: string }[] = [];
  311 |     for (let i = 1; i < options.length; i++) {
  312 |         const value = await options[i].getAttribute('value');
  313 |         const name = await options[i].innerText();
  314 |         if (value) availableDoctors.push({ value, name: name.trim() });
  315 |     }
  316 |     console.log(`[Visit] Found ${availableDoctors.length} doctors to check`);
  317 | 
  318 |     let selectedDoctorValue = '';
  319 | 
  320 |     // 3. Loop through doctors until we find one with targetVisitType
  321 |     for (let idx = 0; idx < availableDoctors.length; idx++) {
  322 |         const doctor = availableDoctors[idx];
  323 |         const doctorStart = Date.now();
  324 |         console.log(`[Visit] [${idx + 1}/${availableDoctors.length}] Trying: "${doctor.name}"...`);
  325 | 
  326 |         const currentDoctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  327 |         await currentDoctorDropdown.waitFor({ state: 'visible', timeout: 3000 });
  328 | 
  329 |         // Select doctor
  330 |         try {
  331 |             await Promise.all([
  332 |                 this.page.waitForLoadState('domcontentloaded', { timeout: 5000 }).catch(() => {}),
  333 |                 currentDoctorDropdown.selectOption(doctor.value)
  334 |             ]);
  335 |             await this.page.waitForTimeout(1500); // Give Angular time to load dropdowns
  336 |         } catch {
  337 |             await this.page.waitForURL('**/VisitScreen*', { timeout: 8000 });
  338 |             await this.page.waitForLoadState('domcontentloaded', { timeout: 5000 });
  339 |             await this.page.waitForTimeout(1500);
  340 |         }
  341 | 
  342 |         // Check if Department auto-loaded
  343 |         const departmentDropdown = this.page.locator('select[ng-model="Visit.DepartmentID"]').first();
  344 |         const deptValue = await departmentDropdown.inputValue().catch(() => '');
  345 | 
  346 |         if (!deptValue || deptValue === '0') {
  347 |             console.log(`⚠ [Visit] Dept empty for "${doctor.name}" - ${Date.now() - doctorStart}ms`);
  348 |             continue;
  349 |         }
  350 | 
  351 |         // Check Visit Type dropdown
  352 |         const visitTypeDropdown = this.page.locator('select[ng-model="Visit.VisitType"]').first();
> 353 |         await this.page.waitForTimeout(2000); // Wait for API to populate
      |                         ^ Error: page.waitForTimeout: Test timeout of 100000ms exceeded.
  354 |         const optionCount = await visitTypeDropdown.locator('option').count().catch(() => 0);
  355 | 
  356 |         if (optionCount <= 1) {
  357 |             console.log(`❌ [Visit] Visit Type empty for "${doctor.name}" - ${Date.now() - doctorStart}ms`);
  358 |             continue;
  359 |         }
  360 | 
  361 |         const types = await visitTypeDropdown.locator('option').allInnerTexts();
  362 |         const cleanTypes = types.map(t => t.trim()).filter(t => t);
  363 | 
  364 |         if (cleanTypes.includes(targetVisitType)) {
  365 |             console.log(`✅ [Visit] FOUND! "${doctor.name}" supports "${targetVisitType}" - total ${Date.now() - startTime}ms`);
  366 |             await visitTypeDropdown.selectOption({ label: targetVisitType });
  367 |             selectedDoctorValue = doctor.value;
  368 |             break; // >>>> STOP LOOP HERE
  369 |         }
  370 | 
  371 |         console.log(`❌ [Visit] "${doctor.name}" has [${cleanTypes.join(', ')}] - ${Date.now() - doctorStart}ms`);
  372 |     }
  373 | 
  374 |     if (!selectedDoctorValue) {
  375 |         throw new Error(`CRITICAL: No doctor found with "${targetVisitType}". Checked ${availableDoctors.length} doctors`);
  376 |     }
  377 | 
  378 |     // 4. After break - Fill Queue No and Admission Source
  379 |     console.log('[Visit] Filling Queue No. and Admission Source...');
  380 | 
  381 |     // Queue No: Random 3-digit number
  382 |     const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  383 |     await expect(queueInput).toBeEnabled({ timeout: 3000 });
  384 |     const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  385 |     await queueInput.fill(randomQueue);
  386 |     console.log(`[Visit] Queue No. set → "${randomQueue}"`);
  387 | 
  388 |     // Admission Source: Select by label
  389 |     const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  390 |     await sourceDropdown.waitFor({ state: 'visible', timeout: 3000 });
  391 |     await sourceDropdown.selectOption({ label: admissionSource });
  392 |     console.log(`[Visit] Admission Source set → "${admissionSource}"`);
  393 | 
  394 |     console.log(`[Visit] fillVisitInformation complete in ${Date.now() - startTime}ms ✅`);
  395 | }
  396 | 
  397 | }
```