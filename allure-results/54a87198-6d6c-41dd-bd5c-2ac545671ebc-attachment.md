      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 18/05/2026 11:43:21"
            - listitem [ref=e13]:
...
```ts
  ...
  279 |     // =====================================================================
  280 |     async fillVisitInformation(encounterType: string, admissionSource: string, targetVisitType: string = 'New') {
  281 |         
  282 |         // 1. Encounter Type
  283 |         const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  284 |         await encounterDropdown.waitFor({ state: 'visible' });
  285 |         await encounterDropdown.selectOption({ label: encounterType }, { force: true });
  286 |         await this.page.waitForTimeout(500);
  287 | 
  288 |         // 2. Extract all available Doctors into memory first
  289 |         const doctorDropdown = this.page.locator('select[ng-model="Visit.DoctorID"]').first();
  290 | await doctorDropdown.locator('option').nth(1).waitFor({ state: 'attached', timeout: 5000 });        
  291 |         const doctorOptionsCount = await doctorDropdown.locator('option').count();
  292 |         const availableDoctors: { name: string, value: string }[] = [];
  293 | 
  294 |         // Skip index 0 (which is usually "--Select--")
  295 |         for (let i = 1; i < doctorOptionsCount; i++) {
  296 |             const option = doctorDropdown.locator('option').nth(i);
  297 |             const name = await option.innerText();
  298 |             const value = await option.getAttribute('value');
  299 |             if (value) availableDoctors.push({ name: name.trim(), value: value });
  300 |         }
  301 | 
  302 |         // 3. DOCTOR -> DEPARTMENT -> VISIT TYPE LOOP
  ...
  306 | 
  307 |         for (const doctor of availableDoctors) {
  308 |             console.log(`Selecting Primary Doctor: ${doctor.name}...`);
  309 | 
  310 |             // Select the doctor to trigger the Department auto-load
  311 |             await doctorDropdown.selectOption({ value: doctor.value }, { force: true });
  312 |             await doctorDropdown.dispatchEvent('change');
  313 | 
  314 |             // Wait 1.5 seconds for the backend to auto-load the Department and Visit Type
> 315 |             await this.page.waitForTimeout(1500);
      |                             ^ Error: page.waitForTimeout: Test timeout of 100000ms exceeded.
  316 | 
  317 |             try {
  318 |                 // Read the loaded Visit Types
  319 |                 const availableVisitTypes = await visitTypeDropdown.locator('option').allInnerTexts();
  320 |                 const cleanVisitTypes = availableVisitTypes.map(v => v.trim());
  321 |                 
  322 |                 // If "New" is an option, select it and STOP THE SCRIPT
  323 |                 if (cleanVisitTypes.includes(targetVisitType)) {
  324 |                     console.log(`✅ Success! "${doctor.name}" allows Visit Type: ${targetVisitType}. Department auto-loaded.`);
  325 |                     
  326 |                     // Select "New"
  327 |                     await visitTypeDropdown.selectOption({ label: targetVisitType }, { force: true });
  328 |                     await visitTypeDropdown.dispatchEvent('change');
  329 |                     
  330 |                     foundWorkingDoctor = true;
  331 |                     
  332 |                     // STOP SEARCHING! This breaks the loop immediately so it doesn't touch other doctors.
  333 |                     break; 
  334 |                 } else {
  335 |                     console.log(`❌ "${targetVisitType}" missing for this doctor. Moving to next...`);
  336 |                 }
  337 |             } catch (error) {
  338 |                 console.log(`❌ Visit Type stayed empty. Moving to next doctor...`);
  339 |             }
  340 |         }
  341 | 
  342 |         // Fail-safe if no doctors support "New"
  343 |         if (!foundWorkingDoctor) {
  344 |             throw new Error(`CRITICAL FAILURE: Could not find ANY doctor that allows the "${targetVisitType}" visit type.`);
  345 |         }
  346 | 
  347 |         // 4. QUEUE NO. (Script jumps here immediately after finding a valid doctor)
  348 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  349 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  350 |         
  351 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  352 |         await queueInput.fill(randomQueue);
  353 |         await this.page.locator('body').click(); // trigger ng-blur
  354 |         await this.page.waitForTimeout(500);
  355 | 
  356 |         // 5. ADMISSION SOURCE
  357 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  358 |         await sourceDropdown.waitFor({ state: 'visible' });
  359 |         await sourceDropdown.selectOption({ label: admissionSource }, { force: true });
  360 |         await sourceDropdown.dispatchEvent('change');
