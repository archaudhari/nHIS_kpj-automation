...
Call log:
  - waiting for locator('select[ng-model="Visit.PatientSourceID"]')
    - locator resolved to <select ng-model="Visit.PatientSourceID" ng-change="PatientSourceChange();" class="form-control input-sm ng-pristine ng-untouched ng-valid" ng-options="item.value as item.text for item in drpPatientSource">…</select>
      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 01:49:45"
            - listitem [ref=e13]:
...
```ts
  455 |                     if (!sel) throw new Error(`Doctor select not found: ${model}`);
  456 |                     sel.value = val;
  457 |                     sel.dispatchEvent(new Event('change', { bubbles: true }));
  458 |                 },
  459 |                 { model: doctorNgModel, val: doctor.value }
  460 |             );
  461 | 
  462 |             await this.page.waitForTimeout(800); // Angular digest: department auto-loads
  463 | 
  464 |             // ── 3a. Log which Department auto-loaded ─────────────────────
  465 |             const deptLabel: string = await this.page.evaluate(() => {
  466 |                 const sel = document.querySelector(
  467 |                     'select[ng-model="Visit.DepartmentID"]'
  468 |                 ) as HTMLSelectElement | null;
  469 |                 return sel?.options[sel.selectedIndex]?.text?.trim() ?? '(not found)';
  470 |             });
  471 |             console.log(`[Visit]   Department auto-loaded → "${deptLabel}"`);
  472 | 
  473 |             // ── 3b. Find Visit Type dropdown ng-model (probe once) ───────
  474 |             const visitTypeNgModel: string = await this.page.evaluate(() => {
  475 |                 const selects = Array.from(document.querySelectorAll('select[ng-model]'));
  476 |                 const match = selects.find(s => {
  477 |                     const m = s.getAttribute('ng-model') ?? '';
  478 |                     return m.toLowerCase().includes('visittype') || m.toLowerCase().includes('visit.type');
  479 |                 });
  480 |                 return match?.getAttribute('ng-model') ?? 'Visit.VisitType';
  481 |             });
  482 | 
  483 |             // ── 3c. Poll Visit Type for real options ─────────────────────
  484 |             try {
  485 |                 await expect.poll(
  486 |                     () => this.page.evaluate((model: string) => {
  487 |                         const sel = document.querySelector(
  488 |                             `select[ng-model="${model}"]`
  489 |                         ) as HTMLSelectElement | null;
  490 |                         return sel ? sel.options.length : 0;
  491 |                     }, visitTypeNgModel),
  492 |                     { timeout: 4000 }
  493 |                 ).toBeGreaterThan(1);
  494 | 
  495 |                 const visitTypes: string[] = await this.page.evaluate((model: string) => {
  496 |                     const sel = document.querySelector(
  497 |                         `select[ng-model="${model}"]`
  498 |                     ) as HTMLSelectElement | null;
  499 |                     return sel
  500 |                         ? Array.from(sel.options).map(o => o.text.trim())
  501 |                         : [];
  502 |                 }, visitTypeNgModel);
  503 | 
  504 |                 console.log(`[Visit]   Available Visit Types: [${visitTypes.join(', ')}]`);
  505 | 
  506 |                 if (visitTypes.includes(targetVisitType)) {
  507 |                     console.log(`✅ [Visit] "${doctor.text}" has "${targetVisitType}" — selecting & stopping.`);
  508 | 
  509 |                     await this.page.evaluate(
  510 |                         ({ model, label }: { model: string; label: string }) => {
  511 |                             const sel = document.querySelector(
  512 |                                 `select[ng-model="${model}"]`
  513 |                             ) as HTMLSelectElement | null;
  514 |                             if (!sel) throw new Error(`VisitType select not found: ${model}`);
  515 |                             const opt = Array.from(sel.options).find(o => o.text.trim() === label);
  516 |                             if (opt) {
  517 |                                 sel.value = opt.value;
  518 |                                 sel.dispatchEvent(new Event('change', { bubbles: true }));
  519 |                             }
  520 |                         },
  521 |                         { model: visitTypeNgModel, label: targetVisitType }
  522 |                     );
  523 | 
  524 |                     await this.page.waitForTimeout(300);
  525 |                     foundWorkingDoctor = true;
  526 |                     break; // ← STOP — do not try more doctors
  527 |                 } else {
  528 |                     console.log(`❌ [Visit] "${targetVisitType}" not available for "${doctor.text}". Trying next...`);
  529 |                 }
  530 | 
  531 |             } catch {
  532 |                 console.log(`❌ [Visit] Visit Type stayed empty for "${doctor.text}". Trying next...`);
  533 |             }
  534 |         }
  535 | 
  536 |         if (!foundWorkingDoctor) {
  537 |             throw new Error(
  538 |                 `[Visit] No doctor supports Visit Type "${targetVisitType}". ` +
  539 |                 `Checked ${availableDoctors.length} doctor(s).`
  540 |             );
  541 |         }
  542 | 
  543 |         // ── 4. QUEUE NO. ────────────────────────────────────────────────
  544 |         const queueInput = this.page.locator('input[ng-model="Visit.TokenNo"]');
  545 |         await expect(queueInput).toBeEnabled({ timeout: 5000 });
  546 |         const randomQueue = Math.floor(100 + Math.random() * 900).toString();
  547 |         await queueInput.fill(randomQueue);
  548 |         await this.page.locator('body').click();
  549 |         await this.page.waitForTimeout(500);
  550 |         console.log(`[Visit] 4. Queue No. → "${randomQueue}"`);
  551 | 
  552 |         // ── 5. ADMISSION SOURCE ─────────────────────────────────────────
  553 |         const sourceDropdown = this.page.locator('select[ng-model="Visit.PatientSourceID"]');
  554 |         await sourceDropdown.waitFor({ state: 'visible', timeout: 5000 });
> 555 |         await sourceDropdown.selectOption({ label: admissionSource });
      |                              ^ Error: locator.selectOption: Test timeout of 100000ms exceeded.
  556 |         await sourceDropdown.dispatchEvent('change');
  557 |         await this.page.waitForTimeout(500);
  558 |         console.log(`[Visit] 5. Admission Source → "${admissionSource}"`);
  559 | 
  560 |         console.log('[Visit] fillVisitInformation complete ✅');
  561 |     }
  562 | }
```