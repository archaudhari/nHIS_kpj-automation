...
Call log:
  ...
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    71 × waiting for element to be visible and enabled
       - element is not visible
     - retrying select option action
       - waiting 500ms
...
> 12 |   await page.selectOption('#LocationID', { label: 'KPJ' });
  ...
  14 | 
  15 |   // Click login button twice
  16 |   await page.click('button:has-text("Login")');
  17 |   await page.waitForTimeout(2000); // short pause before second click
  18 |   await page.click('button:has-text("Login")');
  19 | 
  20 |   // Assert dashboard is visible
  21 |   await expect(page.getByRole('heading', { name: /Patient Dashboard/i })).toBeVisible();
  22 | 
  23 |   // Optional: also verify URL
  24 |   await expect(page).toHaveURL(/PatientDashboard/);
  25 | });
  26 | 
```