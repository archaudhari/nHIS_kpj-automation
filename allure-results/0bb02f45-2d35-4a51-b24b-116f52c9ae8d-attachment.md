      ...
      - navigation [ref=e3]:
        - generic [ref=e4]:
          ...
          - list [ref=e8]:
            ...
            - listitem [ref=e11]:
              - generic [ref=e12]: "Last Login : 20/05/2026 01:35:34"
            - listitem [ref=e13]:
...
```ts
  ...
  296 | ) {
  297 |     // 1. ENCOUNTER TYPE - Outpatient
  298 | const encounterDropdown = this.page.locator('select[ng-model="Visit.EncounterTypeID"]');
  299 | await encounterDropdown.waitFor({ state: 'visible', timeout: 5000 });
  300 | const outpatientOption = { value: "number:1" }; // from your earlier logs
  301 | await encounterDropdown.selectOption(outpatientOption.value);
  302 | 
  303 | // KEY: Wait for Primary Doctor dropdown to be attached after Encounter Type change
  304 | await this.page.waitForLoadState('networkidle', { timeout: 10000 });
  305 | await this.page.waitForTimeout(1000); // let Angular digest
  306 | 
  307 | // 2. PRIMARY DOCTOR - Now it should exist
  308 | const primaryDoctorDropdown = this.page.locator('select[ng-model="Visit.PrimaryDoctorID"]');
> 309 | await primaryDoctorDropdown.waitFor({ state: 'visible', timeout: 15000 }); // bump timeout
      |                             ^ TimeoutError: locator.waitFor: Timeout 15000ms exceeded.
  310 | 
  311 | const doctorOptions = await primaryDoctorDropdown.locator('option').evaluateAll(opts =>
  312 |     opts
  313 |      .map(o => ({
  314 |             value: (o as HTMLOptionElement).value,
  315 |             text: o.textContent?.trim() || ''
  316 |         }))
  317 |      .filter(opt => opt.value && opt.value!== 'string:' && opt.text &&
  318 |              !opt.text.toLowerCase().includes('select'))
  319 | );
  320 | 
  321 | if (doctorOptions.length === 0) {
  322 |     throw new Error('No Primary Doctor options found in dropdown');
  323 | }
  324 | 
  325 | const randomIndex = Math.floor(Math.random() * doctorOptions.length);
  326 | const selectedDoctor = doctorOptions[randomIndex];
  327 | await primaryDoctorDropdown.selectOption(selectedDoctor.value);
  328 | console.log(`[Visit] 2. Primary Doctor set: ${selectedDoctor.text}`);
  329 | }
  330 | }
```