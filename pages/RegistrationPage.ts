...
// =====================================================================
    // REGISTRATION SAVE & CONFIRMATION PROTOCOL
    // =====================================================================
    
    // 1. CLICK MAIN REGISTRATION SAVE BUTTON
    // Using the exact ID "#regbutton" guarantees Playwright only finds ONE button
    const mainSaveButton = this.page.locator('button#regbutton');
    
    await mainSaveButton.waitFor({ state: 'visible', timeout: 15000 });
    await mainSaveButton.scrollIntoViewIfNeeded();
    console.log("Clicking Main Save Button...");
    await mainSaveButton.click();

    // 2. HANDLE THE CONFIRMATION POPUP
    console.log("Waiting for Confirmation Popup...");
    
    // Wait for the specific popup text to appear on the screen
    const confirmDialog = this.page.locator('.ng-confirm-content');
    await expect(confirmDialog).toContainText('Do you want to Save', { timeout: 5000 });

    // Target the specific "Save" span inside the popup and click it
    const popupSaveButton = this.page.locator('span.ng-confirm-btn-text').filter({ hasText: 'Save' });
    await popupSaveButton.waitFor({ state: 'visible' });
    console.log("Confirming Save...");
    await popupSaveButton.click();
    
    // Optional: Wait for the popup to disappear so you know it registered
    await popupSaveButton.waitFor({ state: 'hidden', timeout: 5000 });
